...
export const fetchMoviesAndTVShows = async (
  ...
  try {
    ...
    switch (tvList) {
      case 'popular':
        url = `${TMDBBASEURL}/discover/${mediaType === 'movies' ? 'movie' : 'tv'}?api_key=${API_KEY}&include_adult=false&include_video=false&language=en-US&page=2&sort_by=popularity.desc&without_genres=10749,35&with_genres=10751,16`;
        break;
      case 'top rated':
        url = `${TMDBBASEURL}/discover/${mediaType === 'movies' ? 'movie' : 'tv'}?api_key=${API_KEY}&include_adult=false&include_video=false&language=en-US&page=1&sort_by=vote_average.desc&without_genres=10749,35&vote_count.gte=200&with_genres=10751,16`;
      ...
      case 'on_the_air':
        ...
        break;
      case 'upcoming':
        url = `${TMDBBASEURL}/discover/${mediaType === 'movies' ? 'movie' : 'tv'}?api_key=${API_KEY}&include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=10751,16&without_genres=10749,35&with_release_type=2|3&release_date.gte=${new Date().toISOString()}&release_date.lte=${getDateThreeMonthsAfter()}`;
        break;
